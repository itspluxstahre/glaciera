...
static bool init_xdg_dirs(void) {
    ...
    /* Create directories */
    if (!ensure_dir(xdg_config_dir)) return false;
    if (!ensure_dir(xdg_data_dir)) return false;
    if (!ensure_dir(xdg_cache_dir)) return false;
    
    /* Create themes subdirectory */
    char themes_dir[512];
    snprintf(themes_dir, sizeof(themes_dir), "%s/themes", xdg_config_dir);
    if (!ensure_dir(themes_dir)) return false;
...
void config_set_defaults(config_t *config) {
    ...
    snprintf(config->index_paths[0], sizeof(config->index_paths[0]),
             "%s/Music", home ? home : "/tmp");
    
    snprintf(config->rippers_path, sizeof(config->rippers_path),
             "%s/Music/rippers", home ? home : "/tmp");
    ...
    config->theme.main_bg.r = 46;  config->theme.main_bg.g = 52;  config->theme.main_bg.b = 64;      /* #2e3440 */
    config->theme.main_fg.r = 216; config->theme.main_fg.g = 222; config->theme.main_fg.b = 233;     /* #d8dee9 */
    
    /* Nord Frost (blue accent) */
    config->theme.accent_bg.r = 94;  config->theme.accent_bg.g = 129; config->theme.accent_bg.b = 172;  /* #5e81ac */
    config->theme.accent_fg.r = 236; config->theme.accent_fg.g = 239; config->theme.accent_fg.b = 244;  /* #eceff4 */
    
    /* Nord Aurora (yellow and green) */
    config->theme.playing.r = 235; config->theme.playing.g = 203; config->theme.playing.b = 139;     /* #ebcb8b */
    config->theme.playlist.r = 163; config->theme.playlist.g = 190; config->theme.playlist.b = 140;   /* #a3be8c */
    
    /* Highlight colors */
    config->theme.highlight_bg.r = 236; config->theme.highlight_bg.g = 239; config->theme.highlight_bg.b = 244; /* #eceff4 */
    config->theme.highlight_fg.r = 46;  config->theme.highlight_fg.g = 52;  config->theme.highlight_fg.b = 64;  /* #2e3440 */
}

/* Parse RGB color from TOML table */
static bool parse_color(toml_table_t *table, const char *key, 
                        int *r, int *g, int *b) {
    ...
    if (!rd.ok || !gd.ok || !bd.ok) return false;
...
bool config_load_theme(const char *theme_name) {
    ...
    snprintf(theme_path, sizeof(theme_path), 
             "%s/themes/%s.toml", xdg_config_dir, theme_name);
    ...
    if (colors) {
        parse_color(colors, "main_bg", 
                   &global_config.theme.main_bg.r,
                   &global_config.theme.main_bg.g,
                   &global_config.theme.main_bg.b);
        parse_color(colors, "main_fg",
                   &global_config.theme.main_fg.r,
                   &global_config.theme.main_fg.g,
                   &global_config.theme.main_fg.b);
        parse_color(colors, "accent_bg",
                   &global_config.theme.accent_bg.r,
                   &global_config.theme.accent_bg.g,
                   &global_config.theme.accent_bg.b);
        parse_color(colors, "accent_fg",
                   &global_config.theme.accent_fg.r,
                   &global_config.theme.accent_fg.g,
                   &global_config.theme.accent_fg.b);
        parse_color(colors, "playing",
                   &global_config.theme.playing.r,
                   &global_config.theme.playing.g,
                   &global_config.theme.playing.b);
        parse_color(colors, "playlist",
                   &global_config.theme.playlist.r,
                   &global_config.theme.playlist.g,
                   &global_config.theme.playlist.b);
        parse_color(colors, "highlight_bg",
                   &global_config.theme.highlight_bg.r,
                   &global_config.theme.highlight_bg.g,
                   &global_config.theme.highlight_bg.b);
        parse_color(colors, "highlight_fg",
                   &global_config.theme.highlight_fg.r,
    ...
    if (paths) {
        ...
        if (index_array) {
            ...
            for (int i = 0; i < count && i < MAX_INDEX_PATHS; i++) {
                ...
                if (path.ok) {
                    strncpy(global_config.index_paths[global_config.index_paths_count],
                           path.u.s,
            ...
            if (music.ok) {
                ...
                strncpy(global_config.index_paths[0], music.u.s,
                       sizeof(global_config.index_paths[0]) - 1);
                free(music.u.s);
                printf("Note: Using legacy 'music_library' path. Consider updating to 'index' array.\n");
            }
        }
...
const char *config_get_db_path(void) {
    if (db_path_cache[0] == '\0') {
        snprintf(db_path_cache, sizeof(db_path_cache), 
                "%s/glaciera.db", xdg_data_dir);
    }
